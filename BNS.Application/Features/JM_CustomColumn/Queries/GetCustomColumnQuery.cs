...
{
    public class GetCustomColumnQuery : IRequestHandler<GetCustomColumnRequest, ApiResult<CustomColumnsResponse>>
    {
        ...
        public GetCustomColumnQuery(BNSDbContext context,
        ...
        public async Task<ApiResult<CustomColumnsResponse>> Handle(GetCustomColumnRequest request, CancellationToken cancellationToken)
        {
            var response = new ApiResult<CustomColumnsResponse>();
            response.data = new CustomColumnsResponse();

            var query = _unitOfWork.Repository<JM_CustomColumn>()
                .Where(s => !s.IsDelete && s.CompanyId == request.CompanyId)
                .OrderBy(d => d.CreatedDate)
                .Select(s => _mapper.Map<CustomColumnsResponseItem>(s));
