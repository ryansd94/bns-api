...
namespace BNS.Service.Features
{
    public class UpdateTemplateCommand : UpdateRequestHandler<UpdateTemplateRequest, JM_Template>
    {
        ...
        public override async Task<ApiResult<Guid>> Handle(UpdateTemplateRequest request, CancellationToken cancellationToken)
        {
            ...
            UpdateEntity<JM_Template>(dataCheck, request.ChangeFields, request.UserId);
            ...
            var contentUpdate = request.ChangeFields.Where(s => s.Key.Equals("content")).FirstOrDefault();

            if (contentUpdate != null)
            {
                var contenObjects =JsonConvert.DeserializeObject<ColumnItemRoot>(contentUpdate.Value.ToString());
