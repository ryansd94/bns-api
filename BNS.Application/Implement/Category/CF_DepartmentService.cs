...
{
    public class CF_DepartmentService : GenericRepository<CF_Department>,
        ICF_DepartmentService
    {
        private readonly IGenericRepository<CF_Department> _genericRepository;
        private readonly IStringLocalizer<SharedResource> _sharedLocalizer;
        private readonly IMapper _mapper;
        public CF_DepartmentService(BNSDbContext context,
            IGenericRepository<CF_Department> genericRepository,
        ...
        public async Task<ApiResult<List<CategoryResponseModel>>> GetAllData(RequestPageModel<CategoryResponseModel> model)
        {
            ...
            var query = _context.CF_Departments.Where(s => !string.IsNullOrEmpty(s.Name) &&
            s.IsDelete == null &&
        ...
        public async Task<ApiResult<string>> Save(CF_DepartmentModel model)
        {
            ...
            {
                var checkIsExists = _context.CF_Departments.Where(s => s.Name.ToLower() == model.Name.ToLower() &&
                s.ShopIndex == model.ShopIndex &&
                ...
                rs = await _genericRepository.Add(new CF_Department
                {
                    Name = model.Name,
            ...
            {
                var areaCheck = _context.CF_Departments.Where(s => s.Name.ToLower() == model.Name.ToLower() &&
                s.ShopIndex == model.ShopIndex &&
                s.Index != model.Index &&
                s.IsDelete == null).FirstOrDefault();
                if (areaCheck != null)
        ...
        public async Task<ApiResult<string>> Delete(List<Guid> ids)
        {
            ...
            var data = await _context.CF_Departments.Where(s => ids.Contains(s.Index)).ToListAsync();
        ...
        public async Task<ApiResult<int>> GetMaxNumber(Guid shopIndex)
        {
            ...
            result.data = await GetMaxNumber<CF_Department>(shopIndex,null);
            return result;
        }
